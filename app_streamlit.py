...
def option_selector(label: str, column: str, clean_df: pd.DataFrame, default: Any | None = None) -> Any:
...
def build_reservation_form(clean_df: pd.DataFrame, key_prefix: str) -> dict[str, Any]:
    ...
    with left:
        hotel = option_selector("Hotel", "hotel", clean_df, defaults["hotel"])
        lead_time = st.slider("Dias de anticipacion", 0, 365, 60, key=f"{key_prefix}_lead")
        ...
        arrival_date_month = option_selector("Mes de llegada", "arrival_date_month", clean_df, defaults["arrival_date_month"])
        arrival_date_week_number = st.slider("Semana del anio", 1, 53, defaults["arrival_date_week_number"], key=f"{key_prefix}_week")
        arrival_date_day_of_month = st.slider("Dia del mes", 1, 31, defaults["arrival_date_day_of_month"], key=f"{key_prefix}_day")
        customer_type = option_selector("Tipo de cliente", "customer_type", clean_df, defaults["customer_type"])

    with middle:
        stays_in_weekend_nights = st.slider("Noches de fin de semana", 0, 10, 1, key=f"{key_prefix}_weekend")
        stays_in_week_nights = st.slider("Noches entre semana", 0, 30, 2, key=f"{key_prefix}_weeknights")
        adults = st.number_input("Adultos", 0, 10, 2, key=f"{key_prefix}_adults")
        children = st.number_input("Ninos", 0, 10, 0, key=f"{key_prefix}_children")
        babies = st.number_input("Bebes", 0, 5, 0, key=f"{key_prefix}_babies")
        meal = option_selector("Tipo de comida", "meal", clean_df, defaults["meal"])
        country = option_selector("Pais", "country", clean_df, defaults["country"])

    with right:
        market_segment = option_selector("Segmento de mercado", "market_segment", clean_df, defaults["market_segment"])
        distribution_channel = option_selector("Canal de distribucion", "distribution_channel", clean_df, defaults["distribution_channel"])
        reserved_room_type = option_selector("Habitacion reservada", "reserved_room_type", clean_df, defaults["reserved_room_type"])
        assigned_room_type = option_selector("Habitacion asignada", "assigned_room_type", clean_df, defaults["assigned_room_type"])
        deposit_type = option_selector("Deposito", "deposit_type", clean_df, defaults["deposit_type"])
        adr = st.number_input("Tarifa diaria promedio", -100.0, 10000.0, 90.0, step=5.0, key=f"{key_prefix}_adr")
        required_car_parking_spaces = st.number_input("Parqueaderos requeridos", 0, 8, 0, key=f"{key_prefix}_parking")

    extra_left, extra_right = st.columns(2)
    with extra_left:
        is_repeated_guest = st.toggle("Cliente repetido", value=False, key=f"{key_prefix}_repeated")
        previous_cancellations = st.number_input("Cancelaciones previas", 0, 30, 0, key=f"{key_prefix}_prev_cancel")
        previous_bookings_not_canceled = st.number_input("Reservas previas no canceladas", 0, 100, 0, key=f"{key_prefix}_prev_not_cancel")
    with extra_right:
        booking_changes = st.number_input("Cambios en la reserva", 0, 30, 0, key=f"{key_prefix}_changes")
        days_in_waiting_list = st.number_input("Dias en lista de espera", 0, 400, 0, key=f"{key_prefix}_waiting")
        total_of_special_requests = st.number_input("Solicitudes especiales", 0, 10, 0, key=f"{key_prefix}_requests")
        agent = st.number_input("Codigo de agente", 0.0, 600.0, 0.0, step=1.0, key=f"{key_prefix}_agent")
...
def main() -> None:
    ...
    with tab_new:
        ...
        is_canceled = st.selectbox("Estado real de la reserva", ["Pendiente", "No cancelada", "Cancelada"])
